  ...
  return Object.freeze({
    ...
    async createScreen({ location_id, name = '', resolution = '1920×1080', status = 'draft', active = true }) {
      ...
      const { rows: animationRows } = await pool.query(
        'SELECT enabled, preset_id, profile_json, entity_json, announcement_json, brand_json, environment_json, updated_by FROM animation_settings WHERE id = 1'
      );
      ...
      if (animation) {
        await pool.query(
          ...
          [
            ...
            animation.announcement_json || '{}', animation.brand_json || '{}', animation.environment_json || '{}', animation.updated_by || '', now
          ]
        );
      }
      return getScreen(id);
