...
function buildContext(section, currentPage) {
  ...
  context.innerHTML = `<div class="ui-context-head"><div><span class="ui-context-kicker">TV MENU</span><h2>${contextTitle(section)}</h2></div><button class="ui-context-close" type="button" aria-label="Свернуть панель">‹</button></div><div class="ui-context-body">${contextMarkup(section, currentPage)}</div><div class="ui-account-card"><span class="company-name" data-shell-company></span><div><span class="ui-account-user-row"><strong data-shell-user></strong><button type="button" data-logout>Выйти</button></span><small class="ui-account-role">Панель управления</small></div></div>`;
...
function wireShell(shell, rail, context, section) {
  ...
  rail.querySelectorAll('.ui-rail-button').forEach((link) => {
    link.addEventListener('pointerenter', () => { if (link.classList.contains('active')) setCollapsed(shell, context, false); }, { passive: true });
  ...
  document.querySelector('.main-content')?.addEventListener('pointerdown', () => { if (window.innerWidth <= 1180) setCollapsed(shell, context, true); }, { passive: true });
}
