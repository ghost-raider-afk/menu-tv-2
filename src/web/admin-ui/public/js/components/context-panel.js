...
function linkDescription(href) {
  if (href === '/connect-tv.html') return 'QR, привязка и отключение устройств';
  return '';
}

function contextLinkMarkup([label, href], currentPage) {
  const active = routeIsActive(href, currentPage);
  const description = linkDescription(href);
  const deviceClass = href === '/connect-tv.html' ? ' is-device-route' : '';
  return `<a class="app-route-link${deviceClass}${active ? ' active' : ''}" href="${href}"${active ? ' aria-current="page"' : ''}><span class="app-route-link-copy"><strong>${label}</strong>${description ? `<small>${description}</small>` : ''}</span><span class="app-route-chevron" aria-hidden="true">›</span></a>`;
}

function contextGroupsMarkup(contextGroups, currentPage) {
  return contextGroups.map((group) => {
    const label = group.label ? `<div class="ui-context-group-label">${group.label}</div>` : '';
    return `<section class="ui-context-group">${label}<div class="ui-context-links">${group.links.map((link) => contextLinkMarkup(link, currentPage)).join('')}</div></section>`;
  }).join('');
}
...
export function refreshContextPanel(root = document) {
  ...
  if (!context) return;
  const { section, currentPage, contextGroups } = navigationState();
  ...
  if (body) body.innerHTML = contextGroupsMarkup(contextGroups, currentPage);
...
export function createContextPanel() {
  ...
  context.innerHTML = `<div class="ui-context-head"><div><span class="ui-context-kicker">TV MENU</span><h2>${contextTitle(section)}</h2></div><button class="ui-context-close" type="button" aria-label="Свернуть панель">‹</button></div><div class="ui-context-body">${contextGroupsMarkup(contextGroups, currentPage)}</div><div class="ui-account-card"><span class="company-name" data-shell-company></span><div><span class="ui-account-user-row"><strong data-shell-user></strong><button type="button" data-logout>Выйти</button></span><small class="ui-account-role">Панель управления</small></div></div>`;
