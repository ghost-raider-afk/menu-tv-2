...
function railLink(route, activeSection, extraClass = '') {
  const active = route.key === activeSection;
  return `<a class="ui-rail-button${extraClass ? ` ${extraClass}` : ''}${active ? ' active' : ''}" data-route-section="${route.key}" href="${route.href}" aria-label="${route.label}" title="${route.label}"${active ? ' aria-current="page"' : ''}><span class="ui-rail-icon">${ICONS[route.icon] || ''}</span><span class="ui-rail-label">${route.label}</span></a>`;
}
...
export function createSidebar() {
  ...
  const mobileOverview = railLink({ key: 'overview', label: 'Обзор', href: '/', icon: 'home' }, section, 'ui-rail-mobile-home');
  rail.innerHTML = `<a class="ui-rail-brand" href="/" title="${appName()}"><span class="brand-mark" data-shell-brand>ТВ</span></a><nav class="ui-rail-nav" aria-label="Разделы">${mobileOverview}${PRIMARY_ROUTES.map((route) => railLink(route, section)).join('')}</nav>`;
