...
function promotionEffect(category, overrides) {
  if (overrides.promotion_effect) return overrides.promotion_effect;
  if (category === 'Ambient') return 'glow';
  if (category === 'Promo') return 'pulse-price';
  if (category === 'Dynamic' || category === 'Signature') return 'pulse';
  if (category === 'Premium' || category === 'Visual FX') return 'glow';
  return 'sheen';
}
...
function preset(id, name, category, description, overrides = {}) {
  return Object.freeze({
    ...
    category,
    description,
    profile: Object.freeze({ ...BASE, ...overrides, promotion_effect: promotionEffect(category, overrides) })
  });
}
...
export const ANIMATION_PRESETS = Object.freeze([
  ...
  preset('accent-pulse', 'Пульс акцентов', 'Promo', 'Разделы, акции и цены синхронно дают заметный короткий рекламный импульс.', { pattern:'pulse',flow_direction:'none',cycle_seconds:6.5,event_duration_ms:1250,wave_stagger_ms:0,travel_px:0,scale_amount:0.05,brightness_amount:0.38,section_effect:'pulse',item_effect:'none',price_effect:'pulse',promotion_effect:'pulse-price',intensity:86 }),
  ...
  preset('price-reveal', 'Ценовой ритм', 'Promo', 'Цены и промо-метки дают крупный читаемый импульс — заметно с расстояния без движения строк.', { pattern:'spark',flow_direction:'right-to-left',cycle_seconds:6.5,event_duration_ms:950,wave_stagger_ms:125,travel_px:0,scale_amount:0.06,brightness_amount:0.46,section_effect:'none',item_effect:'none',price_effect:'pulse',promotion_effect:'pulse-price',intensity:90 }),
  ...
  preset('glow-sweep', 'Золотой блик', 'Promo', 'Сильный световой проход по акциям, акцентным зонам и ценам без смещения текста.', { pattern:'spark',flow_direction:'left-to-right',cycle_seconds:7.5,event_duration_ms:1550,wave_stagger_ms:180,travel_px:5,scale_amount:0.022,brightness_amount:0.5,section_effect:'shimmer',item_effect:'none',price_effect:'glow',promotion_effect:'sheen',intensity:94 }),
  preset('quick-snap', 'Quick Pulse', 'Dynamic', 'Короткий мощный импульс разделов, акций и цен с выраженной спокойной паузой.', { pattern:'pulse',flow_direction:'none',easing:'snappy',cycle_seconds:5,event_duration_ms:700,wave_stagger_ms:60,travel_px:0,scale_amount:0.07,brightness_amount:0.46,section_effect:'pulse',item_effect:'none',price_effect:'pop',promotion_effect:'pulse',intensity:94 }),
