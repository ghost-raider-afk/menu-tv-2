...
test('menu editor owns one compact semantic table and one canonical renderer', async () => {
  ...
  assert.match(renderer, /width: 1920/);
  assert.match(renderer, /height: 1080/);
  assert.match(renderer, /tableX: 56/);
  assert.match(renderer, /tableRight: 1430/);
  assert.match(renderer, /tableWidth: 1374/);
  assert.match(renderer, /tableTop: 15/);
  assert.match(renderer, /rowHeight: 53\.5/);
  assert.match(renderer, /secondaryPriceX: 1405/);
  assert.match(renderer, /priceColumnGap: 147/);
  assert.match(renderer, /textAttributes\([^\n]*anchor: 'end'/);
