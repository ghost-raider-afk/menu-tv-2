...
учётная запись и криптографический хеш пароля. Смена пароля выполняется в
«Настройках пользователя» и завершает остальные сессии. Файл `.env` имеет права
`0600` и содержит только системные секреты и настройки контейнеров.
