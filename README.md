...
- временный пароль администратора — ровно 10 символов: строчные и прописные латинские
  буквы, цифра и специальный символ.

В конце установки скрипт один раз показывает адрес и доступы администратора и
базы данных. Сохраните их в менеджере паролей. После успешного запуска временные
логин и пароль администратора удаляются из `.env`: в PostgreSQL остаются только
учётная запись и криптографический хеш пароля. Обычная смена пароля выполняется
в «Настройках пользователя». Если пароль потерян, используйте
`sudo menu-tv-2.0 reset-admin-password [логин]`: новый пароль генерируется по
лимитам `PASSWORD_MIN_LENGTH`, `PASSWORD_MAX_LENGTH` и
`GENERATED_PASSWORD_LENGTH`, показывается в терминале один раз и не записывается
в `.env`. Любая смена или аварийный сброс пароля завершает ранее выданные
веб-сессии этого пользователя. Файл `.env` имеет права `0600` и содержит только
системные секреты и настройки контейнеров.
...
и прописные буквы и цифру. Он показывается один раз; SFTPGo сохраняет хеш
пароля и учётную запись в отдельной PostgreSQL-базе приложения. Учётная запись
имеет доступ только на чтение: просмотр списка и
скачивание.
...
Для локального HTTP-запуска установите в `.env` `SECURE_COOKIES=false`, задайте
собственные безопасные значения для системных секретов и одновременно
`BOOTSTRAP_ADMIN_USERNAME` / `BOOTSTRAP_ADMIN_PASSWORD` для первого запуска.
После
